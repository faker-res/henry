...
angular.module('flowChart', ['dragging'])
    ...
    //
    .directive('flowChart', ['$location', function(location){
        var templateUrl = location.protocol() + "://" + location.host() + ":" + location.port() + "/flowchart/flowchart_template.html";
    ...
    // (which is possible, just not ideal).
    //
    .controller('FlowChartController', ['$scope', '$filter', '$location', 'dragging', '$element', 'socketService',
        function FlowChartController($scope, $filter, $location, dragging, $element, socketService) {
            ...
            $scope.IDArrow = 'url(' + $location.path() + '#idArrow)';
            $scope.NodeBackgroundGradient = 'url(' + $location.path() + '#nodeBackgroundGradient)';
            ...
            this.translateCoordinates = function (x, y, evt) {
                var svg_elem = $element.find("svg")[0];
            ...
            //handle translateion
            //
            $scope.translation = function (input) {
                "use strict";
            ...
            $scope.nodeMouseDown = function (evt, node) {
                ...
                dragging.startDrag(evt, {
                    ...
                    clicked: function () {
                        ...
                        //todo show the node information.
                    }

                });
            };

            $scope.nodeClicked = function (event, data) {
                "use strict";
                socketService.setProposalNodeData(data.data);
            }
            $scope.pointMouseDown = function (evt, point) {
                var chart = $scope.chart;
                var lastMouseCoords;
                ...
                dragging.startDrag(evt, {
                    ...
                    clicked: function () {
                        chart.handlePointClicked(point, evt.ctrlKey);
                    }
                });
            };
            ...
            $scope.connectorMouseDown = function (evt, node, connector, connectorIndex, isInputConnector) {
                ...
                dragging.startDrag(evt, {
                    ...
                    dragStarted: function (x, y) {
                        ...
                        if (node instanceof flowchart.PointViewModel)
                            $scope.dragPoint1 = {x: node.x(), y: node.y()};
                        else
                            $scope.dragPoint1 = flowchart.computeNodeConnectorPos(node, connectorIndex, isInputConnector);
                        $scope.dragPoint2 = {
                            x: curCoords.x,
                            y: curCoords.y
                        };
