    ...
    getPartnerCommConfig: (partnerObjId, commissionType) => {
        ...
        return dbconfig.collection_partner.findOne({_id: partnerObjId}).lean().then(
            ...
            parentData => {
                ...
                if (parentData) {
                    return getDownLineCommConfig(partner._id, partner.platform, parentData._id, commissionType);
                }
                else {
...
function getDownLineCommConfig (partnerObjId, platformObjId, parentObjId, commissionType) {
    let configs = [], providerGroups = [];
