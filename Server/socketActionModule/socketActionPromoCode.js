...
function socketActionPromoCode(socketIO, socket) {
    ...
    this.actions = {
        ...
        modifyPlayerPermissionByPromoCode: function modifyPlayerPermissionByPromoCode(data) {
            let actionName = arguments.callee.name;
            let isValidData = Boolean(data && data.platformObjId && (data.addedPlayerNameArr || data.deletedPlayerNameArr));
            socketUtil.emitter(self.socket, dbPlayerReward.modifyPlayerPermissionByPromoCode, [ObjectId(data.platformObjId), data.addedPlayerNameArr, data.deletedPlayerNameArr], actionName, isValidData);
        },
