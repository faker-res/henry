        ...
        if (conn && wsFunc && dbCall && args && isValid) {
            dbCall.apply(null, args).then(
                function (result) {
                    ...
                    if ((['login','create', 'createGuestPlayer', 'playerLoginOrRegisterWithSMS', 'registerByPhoneNumberAndPassword'].includes(wsFunc.name) &&  wsFunc._service.name === 'player') || conn.playerId && wsFunc.name !== 'getCredit') {
