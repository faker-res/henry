...
describe("Test first top up event", function () {
    ...
    it('get test reward type', function (done) {
        dbRewardType.getRewardType({name: constRewardType.FIRST_TOP_UP}).then(
            function(data){
    ...
    it('Lookup player credit before applying', function () {
        return dbConfig.collection_players.findOne({_id: testPlayerObjId}).select('validCredit').then(
            player => {
    ...
    it('player should apply for first top up reward', function () {
    ...
    it('Test reward task should be created', function () {
        return dbRewardTask.getPlayerCurRewardTask(testPlayerObjId).then(
            function (task) {
                should.exist(task);
                task.type.should.equal('FirstTopUp');
            }
        );
    });
