...
dbconfig.collection_platform.findOne({platformId: platformId}, {_id: 1, topUpSystemType: 1}).lean().then(
    platformData => {
        if (platformData) {
            let playerArr = [];
            let cursor = dbconfig.collection_players.find({platform: platformData._id, isTestPlayer: {$ne: true}}, {_id: 1, name: 1, forbidTopUpType: 1}).cursor();
            let i = 0;
            let done = 0;
            let topUpSystemType;
            ...
            cursor.eachAsync(
                playerData => {
                    ...
                    if (i === 100) {
                        dbPlayerInfo.updatePlayerSubTopupTypePermissionToPMS(platformId, playerArr, topUpSystemType);
                        done += i;
