...
let RewardServiceImplement = function () {
    ...
    this.getLimitedOffers.onRequest = function (wsFunc, conn, data) {
        let isValidData = Boolean(data && data.platformId);
        WebSocketUtil.performAction(conn, wsFunc, data, dbPlayerReward.getLimitedOffers, [data.platformId, ObjectId(conn.playerObjId), data.status], isValidData, false, false, true);
    };
