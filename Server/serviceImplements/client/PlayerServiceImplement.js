...
let PlayerServiceImplement = function () {
    ...
    this.getSMSCode.onRequest = function (wsFunc, conn, data) {
        ...
        WebSocketUtil.performAction(conn, wsFunc, data, dbPlayerMail.sendVerificationCodeToNumber, [conn.phoneNumber, conn.smsCode, data.platformId, captchaValidation, data.purpose, 'player'], isValidData, false, false, true);
    };
    ...
    this.sendSMSCodeToPlayer.onRequest = function (wsFunc, conn, data) {
        ...
        let captchaValidation = conn.captchaCode && data.captcha && conn.captchaCode.toString() === data.captcha.toString();
        conn.captchaCode = null;
        WebSocketUtil.performAction(conn, wsFunc, data, dbPlayerMail.sendVerificationCodeToPlayer, [conn.playerId, smsCode, data.platformId, captchaValidation, data.purpose], isValidData, false, false, true);
    };
