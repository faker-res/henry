...
var PlayerServiceImplement = function () {
    ...
    this.importBIPlayer.onRequest = function (wsFunc, conn, data) {
        var isValidData = Boolean(data && data.name && data.platform);
        if (!isValidData) {
            errorUtils.logMigrationDataInvalidError(this, data);
        }
    ...
    this.loginBIPlayer.onRequest = function (wsFunc, conn, data) {
        ...
        WebSocketUtil.performAction(conn, wsFunc, data, dbMigration.loginBIPlayer, [data.platformId, data.name, data.password], isValidData);
    };
