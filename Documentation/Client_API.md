...
5. [支付信息服务](#支付信息服务：)
	...
	40. [获取玩家『总』有效投注额](#获取玩家(总)有效投注额)	41. [(第三方上下分接口) 快付充值接口](#快付充值接口(第三方上下分接口))
6. [充值意向服务](#充值意向服务：)
	1. [添加充值意向](#添加充值意向)
	2. [修改充值意向](#修改充值意向)
7. [玩家消费记录服务](#玩家消费记录服务：)
...
* **5. 获取有效在线充值方式**
	* 玩家在进入在线充值页面时，需向服务端请求一下玩家可以使用的在线充值方式，来决定显示哪些充值方式。(最好打开页面时查询，因为有些支付方式会根据情况显示不同结果。)
	* name:getOnlineTopupType
	* 请求内容：`{“merchantUse”: 1, “clientType”: 1}`
	* merchantUse: 商户号用途，1-在线商户， 2--银行卡（不用了）
	* clientType: 客户端类型， 1--PC浏览器, 2--h5, 4--app
	* bPMSGroup:true // 是否使用 PMS 分配商户组（默认 fales），使用后 FPMS 后台的分组功能将无效，并且回文的 maxDepositAmount、minDepositAmount 将由 PMS 提供。
	* 响应内容：
		* ```
			{
				status: 200/4xx;
	...
	* topupTypes: 充值方式列表， 该列表列出该玩家在特定客户端(浏览器或手机App)，特定用途(普通充值或开户充值)时，支持的付款方式(参见支付类型表)。
	* type: 支付类型, 1-在线商户是在线支付类型， 2--银行卡时是银行卡类型
	* status: 支付类型的状态，1--正常， 2--系统维护
	* maxDepositAmount: // 单笔最高限额
	* minDepositAmount // 单笔最低限额

<div id='创建在线充值提案'></div>

* **6. 创建在线充值提案**
	* 玩家输入在线充值金额，选择自己的充值方式(通过查询有效充值方式获得)进行充值操作。系统根据不同的充值方式，返回不同的结果。
	* name: createOnlineTopupProposal
	* 请求内容：
		* ```
			{
				topupType:1,
	...
	* bPMSGroup:true // 是否使用 PMS 派商户、个人微信、个人支付宝（默认 fales），使用后将由 PMS 分配派发的银行、支付宝、微信。即与FPMS 的充值分组无关。
	* 响应内容：
		* ```
			{
				status: 200/4xx,
	...
	* status: 操作状态， 200--成功， 4xx--失败
	* data: 提案信息,详细信息如下：
		* serviceCharge：0.0500 //手续费率（玩家实际到帐会扣除手续费）
		* proposalId: 提案Id
		* topupType: 充值方式
		* amount: 充值金额
		* createTime:创建时间
		* status: 提案状态
		* topupDetail: 充值返回的明细信息，不同的充值方式，返回的数据不同。参见[此表](#充值返回)：
	* errorMessage: 详细错误信息
...
|**充值方式**|**返回参数**|**说明**|
|--|--|--|
|NetPay|paymentURL|网银支付跳转地址|
|WechatQR|paymentURL|微信扫码支付跳转地址|
|AlipayQR|paymentURL|支付宝扫码支付跳转地址|
|WechatApp|code<br>appId<br>mchId<br>prepayId<br>apiKey||
|AlipayApp|code<br>product<br>detail<br>price<br>notifyUrl||
...
* **7. 申请手工存款充值**
	* 玩家申请通过手工存款方式完成充值。玩家需先向系统申请一个手工存款申请。系统响应了玩家的申请之后，会生成一个手工存款
	* name:requestManualTopup
	* 请求内容：
		* ```
			{
				bankTypeId: “1”,
	...
	* lastBankcardNo: 手工充值银行卡后几位数 （银行卡转账需要填）
	* amount: 手工存款金额 (必填项)
	* depositMethod: 手工存款方式 (必填项)
	* realName: 银行卡姓名 （柜台银行卡转账需要玩家自己填）
	* provinceId: 玩家手工充值所在省Id
	* cityId: 玩家手工充值所在城市Id
	* districtId: 玩家手工充值所在区/县Id
	* bonusCode: 优惠代码
	* remark: （柜台现金转账需要填，放playerId）
	* limitedOfferObjId: (可选)指定充值应用于哪个秒杀礼包
	* topUpReturnCode: (可选) 指定充值应用于哪个秒存送金
	* bPMSGroup: // 是否使用pms银行卡组，默认false (补充：如果开启了 FPMS 进行财务操作，会忽略此参数，直接由 FPMS 派自创的卡）
	* supportMode: “new”/”old” 是否使用新的pms分组模式，不发送该参数为旧的模式
	* 响应内容：
		* ```
			{
				status: 200/4xx,
	...
	* status: 操作状态， 200--成功，4xx--失败
	* data: 申请到的手工存款单信息， 内容如下：
	* proposalId: 提案Id
	* requestId: 申请单Id
	* Status: 提案状态
	* Result: 申请到的银行卡信息, 内容如下：
	* bankTypeId: 充值目标银行类型Id
	* bankName: 银行名称
	* bankCardNo: 充值目标银行卡号
	* cardOwner: 充值目标银行户名
	* createTime: 申请时间
...
<div id='取消手工存款申请单'></div>

* **8. 取消手工存款申请单**
	* 玩家可以取消已申请到的手工存款申请单。
	* name:cancelManualTopupRequest
	* 请求内容：`{proposalId: “8538598”}`
	* proposalId: 提案Id
	* 响应内容：`{status: 200/4xx, data : {proposalId: “8538598”}, errorMessage: “xxxxxxx”}`
	* Status:操作状态， 200--成功, 4xx--失败
	* proposalId: 提案号
	* errorMessage: 详细错误信息

<div id='延长手工存款申请单有效时间'></div>

* **9.  延长手工存款申请单有效时间 (功能待定,可先不实现)**
	* 玩家手工存款超时，请求延长手工存款申请单的保留时间。
	* name:delayManualTopupRequest
	* 请求内容：`{proposalId: “0001”, delayTime: 30}`
	* proposalId: 提案号的Id
	* delayTime: 延长时间数（分钟）
	* 响应内容：
		* ```
	...
	* Status: 操作状态， 200--操作成功， 4xx--操作失败
	* proposalId与delayTime用于进行路由

<div id='修改手工存款申请单信息'></div>

* **10.  修改手工存款申请单信息**
	* 玩家可以修改手工存款充值申请单的信息，如存款金额。
	* name:modifyManualTopupRequest
	* 请求内容：
		* ```
			{
	...
	*	proposalId: 提案Id
	*	bankTypeId: 手工充值银行类型
	*	lastBankcardNo: 手工充值银行卡后几位数
	*	amount: 手工存款金额
	*	providerId: 玩家手工充值所在省Id
	*	cityId: 玩家手工充值所在城市Id
	*	districtId: 玩家手工充值所在区/县Id
	*	响应内容：
		*	```
...
<div id='获取手工存款申请单列表'></div>

* **12.  获取手工存款申请单列表**
	* 获取玩家申请的手工存款申请单. (一般不允许玩家同时申请多个手工存款单) .
	* name:getManualTopupRequestList
	* 请求内容：`{requestId: “8943835”}`
	* requestId: 请求Id，用于路由。
	* 响应内容：
		* ```
			{
				...
				"data": {
					...
					"type": "57b6ca35c50087d95c9cf35c",
					"mainType": "TopUp",  提案类型
					"status": "PrePending",  提案状态
					"__v": 0,
					"noSteps": true,
					"userType": "2",
					"entryType": "1",
					...
					"data": {  提案详细内容
						...
						"bankName": "3",  银行名称
						"bankAccount": "2374827346267423",  银行账号
						"bankAccountName": "vince",  账号名称
						“validTime: “”
						"creator": {
							"type": "player",
							"name": "vince1",
					...
					"createTime": "2016-10-13T06:26:17.068Z" 创建时间
				}
			}
	* Status: 操作状态, 200--成功， 4xx--失败
	* requestId: 请求Id, 路由之用
	* Data: 手工存款申请单列表, 明细信息参见requestManualTopup的data字段
	* errorMessage: 详细错误信息


<div id='获取支付宝存款申请单列表'></div>

* **12.  获取支付宝存款申请单列表**
	* 获取玩家申请的支付宝存款申请单. (一般不允许玩家同时申请多个手工存款单) .
	* name:getAlipayTopupRequestList
	* 请求内容：`{requestId: “8943835”}`
	* requestId: 请求Id，用于路由。
	* 响应内容：
		* ```
			{
				...
				"data": {
					"proposalId": "vp400180",
					"requestId": "99994298578",  提案id
					"status": "Pending",  提案状态
					"result": {
						"requestId": "99994298578",
						"bankTypeId": "170",
						"alipayAccount": "1234123412341456",  支付宝账号
						"alipayQRCode": "[http://img99.neweb.me/pms/2016_12_5_2094566462.jpeg](http://img99.neweb.me/pms/2016_12_5_2094566462.jpeg)",
						支付宝QR code
						"amount": 246  额度
					}
				}
...
<div id='获取微信存款申请单列表'></div>

* **13.  获取微信存款申请单列表**
	* 获取玩家申请的微信存款申请单. (一般不允许玩家同时申请多个手工存款单) .
	* name:getWechatTopupRequestList
	* 请求内容：`{requestId: “8943835”}`
	* requestId: 请求Id，用于路由。
	* 响应内容：
		* ```
			{
				...
				"data": {
					...
					"result": {
						"requestId": "99994298578",
						"bankTypeId": "170",
						"wechatAccount": "1234123412341456",  微信账号
						"wechatQRCode": "[http://img99.neweb.me/pms/2016_12_5_2094566462.jpeg](http://img99.neweb.me/pms/2016_12_5_2094566462.jpeg)",
						微信QR code
						"amount": 246  额度
					}
				}
...
<div id='申请支付宝转账充值'></div>

* **14.  申请支付宝转账充值**
	* 玩家申请使用支付宝转账的方式进行充值。玩家需要先向系统申请一个支付宝账号作为转账对象。玩家只能申请一个有效支付宝转账申请单，在完成或取消申请单之后，才能再申请支付宝转账申请单。
	* Name: requestAlipayTopup
	* 请求内容：
		* ```
			{
				amount: 100,
	...
	* limitedOfferObjId: (可选) 指定充值应用于哪个秒杀礼包
	* topUpReturnCode: (可选) 指定充值应用于哪个秒存送金
	* bPMSGroup:true // 是否使用 PMS 派商户、个人微信、个人支付宝（默认 fales），使用后将由 PMS 分配派发的银行、支付宝、微信。即与FPMS 的充值分组无关(补充：如果开启了 FPMS 进行财务操作，会忽略此参数，直接由 FPMS 派自创的支付宝帐号）。
	* 响应内容：
		* ```
			{
				status:200/4xx,
				data:{
					...
					result: {
						R
						alipayAccount:”13144820853”’,
						alipayQRCode:”[http://img99.neweb.me/pms/2016_12_6_380151071.jpeg](http://img99.neweb.me/pms/2016_12_6_380151071.jpeg)”,
						amount:”666”,
						bankTypeId:”170”
						line:1 //个人支付宝线路，1：PMS自己提供的服务， 2：第三方提供的个人支付宝线路
					}
				}
			}
	* status:200:成功 / 4xx:失败
...
* **15.  申请微信转账充值**
	* Name: requestWechatTopup
	* 请求内容：`{amount: 100，bonusCode: 优惠代码，clientType: 2}`
	* notUseQR: (默认false)
	* limitedOfferObjId: (可选) 指定充值应用于哪个秒杀礼包
	* topUpReturnCode: (可选) 指定充值应用于哪个秒存送金
	* bPMSGroup:true // 是否使用 PMS 派商户、个人微信、个人支付宝（默认 fales），使用后将由 PMS 分配派发的银行、支付宝、微信。即与FPMS 的充值分组无关(补充：如果开启了 FPMS 进行财务操作，将会忽略此参数，直接由 FPMS 派自创的微信）。
	* 响应内容：
		* ```
			{
				status:200/4xx,
				data:{
					...
					status: “Pending”,
					result: {
						"requestId": "15821189046",
						"bankTypeId": "171",
						"weChatAccount": "tomashantest",
						“weChatQRCode” : “xxxxx”
						"amount": 100,
						"createTime": "2017-04-18 14:06:41",
						"validTime": "2017-04-18 15:06:42"
					}
				}
			}
	* status:200:成功 / 4xx:失败
	* proposalId: 微信转账充值单提案Id
	* amount: 充值金额
	* Status:充值提案状态
...
* **16.  申请秒付宝充值**
	* Name: requestQuickpayTopup
	* 请求内容：`{amount: 100, quickpayName: xxx}`
	* 响应内容：
		* ```
			{
				status:200/4xx,
				data:{
					...
					status: “Pending”,
					result: {
						mfgAccount:”13144820853”’,
						mfbQRCode:”[http://img99.neweb.me/pms/2016_12_6_380151071.jpeg](http://img99.neweb.me/pms/2016_12_6_380151071.jpeg)”,
						Amount:”666”,
					}
				}
			}
	* status:200:成功 / 4xx:失败
	* proposalId: 秒付宝转账充值单提案Id
	* amount: 充值金额
	* status:充值提案状态

<div id='取消支付宝转账充值'></div>

* **17.  取消支付宝转账充值**
	* Name: cancelAlipayTopup
	* 请求内容：`{proposalId: xxx}`
	* 响应内容：`{status:200/4xx, data: {proposalId: “867437854”} / errorMessage: xxx}`
	* status:200:成功 / 4xx:失败
	* proposalId: 支付宝转账充值单提案Id
...
* **18.  取消微信转账充值**
	* Name: cancelWechatTopup
	* 请求内容：`{proposalId: xxx}`
	* 响应内容：`{status:200/4xx, data: {proposalId: “867437854”} / errorMessage: xxx}`
	* status:200:成功 / 4xx:失败
	* proposalId: 微信转账充值单提案Id

<div id='手工存款充值状态通知'></div>

* **19.  手工存款充值状态通知**
	* 手工存款充值单状态发生改变时，会向在线的玩家主动发送充值状态更新的通知.
	* name:manualTopupStatusNotify
	* 响应内容：
		* ```
			{
				status:200/4xx,
...
* **20.  在线充值状态通知**
	* 在线充值提案单状态发生改变时，会向在线玩家主动发送充值状态更新的通知。
	* name:onlineTopupStatusNotify
	* 响应内容：
		* ```
			{
				status:200/4xx,
