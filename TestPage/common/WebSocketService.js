...
///-----------------------WebSocketService  Start-------------------------------
    ...
    proto._sendRequest = function (funcName, data) {
        ...
        console.log( "_sendRequest, packageData", JSON.stringify(packageData) );
        if (typeof $ === 'function') {
            $('#requestMessage').text( JSON.stringify(data, null, 2) );
            if (window.textareaToDisplayRequest) {
                $(window.textareaToDisplayRequest).text( JSON.stringify(data, null, 2) );
                if (window.TestHelper) {
                    window.TestHelper.displayDataInTextArea(data, window.textareaToDisplayRequest);
                }
            }
        }
    };
     ...
     * @private
     */
    proto._sendResponse = function (conn, funcName, data) {
        if (!funcName || !conn)
            return;
        var packageData = {service: this.name, functionName: funcName, data: data};
